...
const ENDPOINT_VERSION = 'vera-video-direct-v2';
...
async function fetchEndpointHealth() {
  ...
  try {
    ...
    if (Number(data.maxVideoBytes) < MAX_VIDEO_BYTES) throw new Error('GoDaddy video endpoint reports an outdated video size limit.');
    return data;
  } finally {
    clearTimeout(timer);
  }
