(() => {
  ...
  async function loadImageSource(file) {
    // Android Chrome is more reliable with a normal <img> element than
    // createImageBitmap() for some camera-generated JPEG files.
    try {
      return await loadWithImageElement(file);
    } catch (_) {
      // Fall back to createImageBitmap for browsers/files that prefer it.
    }
    ...
    if (ext === 'heic' || ext === 'heif' || file.type === 'image/heic' || file.type === 'image/heif') {
      throw new Error('This HEIC/HEIF photo cannot be opened by this Android browser. Please choose a JPEG, PNG, or WebP photo.');
    }
